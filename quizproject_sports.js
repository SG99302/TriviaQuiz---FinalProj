...
function changeData(data) {
   ...
   },
   {
	...
	"question_number": "10",
	"a": data.results[9].correct_answer,
	"b": data.results[9].incorrect_answers[2],
	"c": data.results[9].incorrect_answers[1],
	"d": data.results[9].incorrect_answers[0],
	"answer": "a",
 ...
 function markIt(ans) {
    ...
    if (currentQuestion >= questions.length) {
       ...
       message = "The quiz is over! Final Score: " + score + " / " + questions.length;
	   ...
	   message += "<br><br>" + "What would you like to do next?" + "<br>" + "<button id='retry' onclick='retry()'>Play Again</button>" + "&nbsp;" + "<button id='cool' onclick='home()'>Go to Home</button>" + "&nbsp;" + "<button id='good' onclick='gk()'>General Knowledge Quiz</button>";
    } else {
       loadQuestion();
    }
...
function timer() {
  ...
  if(timeleft <= -1){
	...
	+ "<button id='good' onclick='gk()'>General Knowledge Quiz</button>";
...
function gk() {
	window.location.href = 'generalknowledge.html';
}