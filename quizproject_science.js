 ...
 function markIt(ans) {
    ...
    if (currentQuestion >= questions.length) {
	   ...
	   if(score == 0 || score == 1) {
	    ...
	    message += "<br><br>" + "What would you like to do next?" + "<br>" + "<button id='retry' onclick='retry()'>Play Again</button>" + "&nbsp;" + "<button id='cool' onclick='home()'>Go to Home</button>" + "&nbsp;" + "<button id='good' onclick='math()'>Math Quiz</button>";
    } else {
       loadQuestion();
    }
...
function timer() {
  ...
  if(timeleft <= -1){
	...
	+ "<button id='good' onclick='math()'>Math Quiz</button>";
