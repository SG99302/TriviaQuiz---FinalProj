...
function changeData(data) {
 ...
 function markIt(ans) {
    ...
    if (currentQuestion >= questions.length) {
	   ...
	   message += "<br><br>" + "What would you like to do next?" + "<br>" + "<button id='retry' onclick='retry()'>Play Again</button>" + "&nbsp;" + "<button id='cool' onclick='home()'>Go to Home</button>" + "&nbsp;" + "<button id='good' onclick='sport()'>Sports Quiz</button>";
    } else {
       loadQuestion();
    }
...
function timer() {
  ...
  if(timeleft < 0){
	...
	+ "<button id='good' onclick='sport()'>Sports Quiz</button>";
